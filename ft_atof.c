...
void	ft_init_and_parse_sign(t_atof_data *data, const char **str)
{
	data->result = 0.0;
	data->fraction = 0.0;
	data->sign = 1.0;
...
void	ft_parse_integer_and_fraction(t_atof_data *data, const char **str)
{
	while (**str >= '0' && **str <= '9')
	{
...
void	ft_parse_exponent(t_atof_data *data, const char **str)
{
	if (**str == 'e' || **str == 'E')
	{
		(*str)++;
...
double	ft_finalize_result(t_atof_data *data)
{
	int	i;
	double result;
